...
function createSerpent() {
    ...
    function littleEndianToInt(key, off) {
        var n = key[off] & 0xff;
        n |= (key[++off] & 0xff) << 8;
        n |= (key[++off] & 0xff) << 16;
        n |= (key[++off] & 0xff) << 24;
    ...
    function intToLittleEndian(n,key,off) {
        key[off] = n & 0xff;
        key[++off] = (n >>> 8) & 0xff;
        key[++off] = (n >>> 16) & 0xff;
        key[++off] = (n >>> 24) & 0xff;
    ...
    function makeWorkingKey(key) {
        var kPad = new Array(8);
        var off = 0;
        var length = 0;
        var i;
        var wKey;
        ...
        for (off = 0; (off + 4) < key.length; off += 4) {
            kPad[length++] = littleEndianToInt(key, off);
        }
        ...
        if (off % 4 == 0) {
            ...
            alert ("Key must be a multiple of 4 bytes");
        }
        
        var amount = (ROUNDS + 1) * 4;
        var w = new Array(amount);
        
        for (i = 0; i < 8; i ++) {
            kPad[i] = rotateLeft(kPad[i]^kPad[(i+3)%8]^kPad[(i+5)%8]^kPad[(i+7)%8]^PHI^(i),11);
        }
        for (i = 0; i < 8; i ++) {
            w[i] = kPad[i];
        }
    ...
    function srpInit(paramKey) {
        wKey = makeWorkingKey(paramKey);
    }
    ...
    function srpEncrypt2( data,offset)
    {
      dataBytes = data;
      dataOffset = offset;
    ...
    function srpEncrypt(data,offset) {
        X0 = littleEndianToInt(data, offset);
        X1 = littleEndianToInt(data, offset + 4);
        X2 = littleEndianToInt(data, offset + 8);
        X3 = littleEndianToInt(data, offset + 12);
        ...
        intToLittleEndian(wKey[128] ^ X0, data, offset);
        intToLittleEndian(wKey[129] ^ X1, data, offset + 4);
        intToLittleEndian(wKey[130] ^ X2, data, offset + 8);
        intToLittleEndian(wKey[131] ^ X3, data, offset + 12);

    }
    function srpDecrypt(data,offset) {
        X0 = wKey[128] ^ littleEndianToInt(data, offset);
        X1 = wKey[129] ^ littleEndianToInt(data, offset + 4);
        X2 = wKey[130] ^ littleEndianToInt(data, offset + 8);
        X3 = wKey[131] ^ littleEndianToInt(data, offset + 12);
                ib7(X0, X1, X2, X3);
        ...
        intToLittleEndian(X0 ^ wKey[0], data, offset);
        intToLittleEndian(X1 ^ wKey[1], data, offset + 4);
        intToLittleEndian(X2 ^ wKey[2], data, offset + 8);
        intToLittleEndian(X3 ^ wKey[3], data, offset + 12);
    }
    function srpDecrypt2(data,offset)
    {
      dataBytes = data;
      dataOffset = offset;
      var blk = dataBytes.slice(dataOffset,dataOffset+16); blk.reverse();
